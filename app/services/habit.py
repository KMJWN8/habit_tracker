...
class HabitService:
    def __init__(self, habit_repository: HabitRepository):
        ...
        async def create_habit(self, user: User, habit_data: HabitCreate) -> Habit:
        ...
        async def get_user_habit(self, user: User, habit_id: int) -> Optional[Habit]:
        ...
        async def update_habit(
            self, user: User, habit_id: int, habit_data: HabitUpdate
        ) -> Optional[Habit]:
            ...
            if updated_habit:
                ...
                logger.warning(
                    f"Habit {habit_id} not found for update for user: {user.id}"
                )

            return updated_habit

        async def delete_habit(self, user: User, habit_id: int) -> bool:
            success = await self.habit_repository.delete_user_habit(user.id, habit_id)

            if success:
                logger.info(
                    f"Habit {habit_id} deleted (deactivated) for user: {user.id}"
                )
            else:
                logger.warning(
                    f"Habit {habit_id} not found for deletion for user: {user.id}"
                )
